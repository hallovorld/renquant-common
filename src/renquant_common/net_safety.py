...
Lifted from ``RenQuant/backtesting/renquant_104/kernel/net_safety.py``
(umbrella) to renquant-common (this repo) per CLAUDE.md §3.5 — shared
HTTP retry/budget infrastructure belongs in renquant-common, not in
the umbrella as canonical source. Consumers across renquant-base-data
(8 sites) and renquant-pipeline (4 sites) now import from this home.
...
Usage:
    ...
    from renquant_common.net_safety import call_with_timeout, FetchBudget
...
log = logging.getLogger("renquant_common.net_safety")
